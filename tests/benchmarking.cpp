...
const uint64_t SMALL_SIZE = pow(2, 10);
...
double lmap2_insert_speed(LinearMap2<uint64_t, uint64_t> &lmap2, std::vector<uint64_t> keys){
    auto t1 = steady_clock::now();
    for(uint64_t key : keys){
        lmap2.insert(key, 1);
    }
...
double lmap2_emplace_speed(LinearMap2<uint64_t, uint64_t> &lmap2, std::vector<uint64_t> keys){
    uint64_t value;
    uint64_t sum = 0;
    auto t1 = steady_clock::now();
    for(uint64_t key : keys){
...
double lmap2_remove_speed(LinearMap2<uint64_t, uint64_t> &lmap2, std::vector<uint64_t> keys){
    auto t1 = steady_clock::now();
    for(uint64_t key : keys){
        lmap2.remove(key);
    }
...
void benchmark_insert_speed(){
    ...
    benchmark_insert << "Num Entries,LMap+TS,LMap+BS,RMap,UMap\n";

    for(uint64_t i = STEP_SIZE; i <= MAX_ELEM; i+=STEP_SIZE){
        // Declare hashmaps
        LinearMap2<uint64_t, uint64_t> lmap2(SMALL_SIZE);
...
void benchmark_emplace_speed(){
    ...
    benchmark_emplace << "Num Entries,LMap+TS,LMap+BS,RMap,UMap\n";
...
void benchmark_remove_speed(){
    ...
    benchmark_remove << "Num Entries,LMap+TS,LMap+BS,RMap,UMap\n";
...
void benchmark_insert_with_removal_speed(){
    ...
    benchmark_insert_with_removal << "Num Entries,LMap+TS,LMap+BS,RMap,UMap\n";
...
void benchmark_emplace_with_removal_speed(){
    ...
    benchmark_emplace_with_removal << "Num Entries,LMap+TS,LMap+BS,RMap,UMap\n";
...
void benchmark_avg_PSL(){
    ...
    benchmark_PSL << "Num Entries,LMap+TS,LMap+BS,RMap\n";

    for(uint64_t i = STEP_SIZE; i <= MAX_ELEM; i+=STEP_SIZE){
        // Declare hashmaps
        LinearMap2<uint64_t, uint64_t> lmap2(SMALL_SIZE, 0.95);
        LinearMap<uint64_t, uint64_t> lmap(SMALL_SIZE, 0.95);
        RobinhoodMap<uint64_t, uint64_t> rmap(SMALL_SIZE, 0.95);
...
void benchmark_memory_usage(){
    ...
    benchmark_memory << "Num Entries,LMap+TS,LMap+BS,RMap\n";
    ...
    for(uint64_t i = STEP_SIZE; i <= MAX_ELEM; i+=STEP_SIZE){
        ...
        LinearMap<uint64_t, uint64_t> lmap(SMALL_SIZE);
        RobinhoodMap<uint64_t, uint64_t> rmap(SMALL_SIZE);
        std::unordered_map<uint64_t, uint64_t> umap(SMALL_SIZE);
