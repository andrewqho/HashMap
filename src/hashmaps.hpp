...
class RobinhoodMap
{
        ...
        void insert(K const &key, V const &value){
            ...
            while(!entries[idx].isClean()){
                ...
                if(entries[idx].getPSL() < PSL){
                    swap_entries(entries[idx], key_to_write, val_to_write, hash_val, PSL);
                }
    ...
    private: 
        ...
        size_t capacity;
        H hash_func;
        void swap_entries(Entry<K, V> &entry, K& key, V& val, size_t& hash_val, size_t& PSL){
            std::swap(entry.getKey(), key);
            std::swap(entry.getValue(), val);
            std::swap(entry.getHash(), hash_val);
            std::swap(entry.getPSL(), PSL);
        }
        void reallocate(){
